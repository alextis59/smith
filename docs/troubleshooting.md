...
`--read-only` blocks common write commands such as redirects, `touch`, `rm`, `mv`, `sed -i`, package installs, the provider `patch` tool, and `smith_patch`. It is intended for inspection tasks and can block legitimate commands that have unusual syntax.
