...
function finishReportsConcreteBlocker(message: string): boolean {
  return /\b(?:permission denied|not writable|read-only|missing (?:dependency|package|tool|service|credential|credentials|sample|fixture|example|output|data)|(?:dependency|sample|fixture|example|output|data) (?:is )?missing|(?:command|binary|tool) (?:is )?(?:unavailable|not available|not installed|not found)|(?:workspace|environment|repo|repository) (?:has|contains|provides) no (?:[^.\n]{0,80} )?(?:command|binary|tool|sample|fixture|example|output|data)|environment (?:issue|limitation|does not support|is missing)|network (?:unavailable|blocked|failure)|access (?:denied|blocked|unavailable)|requires? (?:user|manual|external|network|credential|credentials|sample|fixture|example|output|data)|cannot (?:verify|derive|inspect|determine) (?:[^.\n]{0,80} )?(?:format|schema|sample|output|data)|cannot continue because (?:the )?(?:required )?(?:dependency|tool|service|credential|credentials|environment|sample|fixture|example|output|data)|not practical (?:in|with|without) (?:this|the) (?:environment|workspace|available tools)|impossible (?:in|with|without) (?:this|the) (?:environment|workspace|available tools))\b/i.test(
    message
  );
}
